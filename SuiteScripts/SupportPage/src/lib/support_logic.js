...
define([], function () {
  ...
  // Runs before <body> renders so the page never flashes light when the
  // user has previously chosen dark (or has no preference — dark is default).
   ...
   *   state: { submitted: bool, name: string, email: string, ticketId: string, baseUrl: string }
   */
  function renderPage(state) {
    state = state || {};
    var base = escapeHtml(state.baseUrl || "");
    var ticketsUrl = base + "&view=tickets";
    ...
    var formSection = state.submitted ? "" : [
      '    <section class="card">',
    ...
    var charCounterScript = state.submitted ? "" : [
      "  <script>",
      "    (function () {",
    ...
    var urgencyPulseScript = state.submitted ? "" : [
      "  <script>",
      "    (function () {",
   ...
   *   state: { tickets: Array<{ticketId, name, email, topic, date}>, baseUrl: string }
   */
  ...
  return {
    ...
    renderTicketsPage: renderTicketsPage
  };
});
